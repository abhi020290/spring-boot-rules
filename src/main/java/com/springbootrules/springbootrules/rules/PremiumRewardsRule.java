...
@Rule(name = "PremiumRewardsRule", description = "Identify the type of users", priority = 1)
@Slf4j
public class PremiumRewardsRule {

    @Condition
    public boolean checkCondition(@Fact("rewardsRequest") RewardsRequest rewardsRequest) {
        return Integer.parseInt(rewardsRequest.getAmount()) > 100
                && rewardsRequest.getUserType().equalsIgnoreCase("PREMIUM");
    }

    @Action
    public void takeAction(Facts facts) {
        log.info("Invoked PremiumRewardsRule............");
        RewardsRequest rewardsRequest = facts.get("rewardsRequest");
        RewardsResponse rewardsResponse = facts.get("rewardsResponse");
        long points = (long) (Integer.parseInt(rewardsRequest.getAmount()) * 0.1);
        long valueOfPoints = points * 10;
        rewardsResponse.setPoints(points);
        rewardsResponse.setPointsAsValue(valueOfPoints);
        rewardsResponse.setWhichRule("PremiumRewardsRule");
    }
